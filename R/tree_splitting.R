...
Node <- R6Class("Node", list(
    ...
    computeSplit = function(X, Y, objective, impr.par, optimizer, min.split = 10) {
      ...
      if (length(self$subset.idx) < min.split | self$improvement.met == TRUE) {
        ...
        if ( rsqrt - self$rsqrt < self$improvement*impr.par){
    ...
    computeChildren = function(X, Y, objective) {
      ...
      if (self$stop.criterion.met) {
        ...
        left.child = Node$new(id = 1, depth = self$depth + 1, subset.idx = idx.left, id.parent = self$id, child.type = "<=", objective.value = obj.left, improvement.met = self$improvement.met, rsqrt = self$rsqrt, improvement = self$improvement)
        right.child = Node$new(id = 2, depth = self$depth + 1, subset.idx = idx.right, id.parent = self$id, child.type = ">", objective.value = obj.right, improvement.met = self$improvement.met, rsqrt = self$rsqrt, improvement = self$improvement)
